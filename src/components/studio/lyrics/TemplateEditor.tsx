      ...
      <div className="flex flex-wrap gap-1">
        {rows.map((row) => (
          <Chip
            key={row.name}
            token={`{{${row.name}}}`}
            title={`Insert the ${row.label.toLowerCase()}`}
          >
            + {row.label}
          </Chip>
        ))}
      ...
      {rows.map((row) => (
        ...
        >
          ...
          <div className="flex flex-wrap gap-1">
            <Chip
              token={`{{${row.name}}}`}
              title={`Once for each language, in its own: ${langs.join(", ")}`}
            >
              All
            </Chip>
            ...
            {langs.map((lang, index) => (
              <Chip
                key={index}
                token={`{{${row.name}:${index + 1}}}`}
                title={`Language ${index + 1} — ${lang}`}
              >
                {index + 1}
              </Chip>
            ))}
...
const nameOf = (element: TemplateElement): string => {
  if (element.kind === "text")
    return element.content.replace(/\s+/g, " ").trim() || "Empty text";
  ...
  <>
    ...
    <Row label="Size" stack>
      <Select
        ...
        onChange={(value) =>
          patch({ autoSize: value === "fixed" ? "fixed" : "shrink" })
        }
        options={[
      ...
      <Toggles
        ...
        options={[
          ...
          { value: "color" as const, label: "A flat colour", Icon: Square },
          {
            ...
            label: "A gradient between two colours",
            Icon: Blend,
          },
        ]}
    ...
    {style.plateKind === "gradient" ? (
      <GradientRows
        value={style.plateGradient}
        onChange={(plateGradient) => patch({ plateGradient })}
      />
    ) : null}
        ...
        <Row label="Behind">
          <Toggles
            ...
            options={[
              {
                value: "box" as const,
                label: "One panel behind the whole box",
                text: "The box",
              },
              {
                value: "line" as const,
                label: "A band behind each line",
                text: "Each line",
              },
            ]}
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          <Toggles
            ...
            options={[
              ...
              {
                value: true,
                label: "Drop commas, periods and the like",
                text: "Ignore",
              },
            ]}
      ...
      <Row label="Line breaks">
        <Toggles
          ...
          options={[
            ...
            {
              value: true,
              label: "Keep every break as it was typed",
              text: "Keep as typed",
            },
          ]}
          ...
          options={[
            {
              ...
              label: "Stack the languages in this box",
              text: "Stacked",
            },
            {
              value: "split" as const,
              label: "Give each language a share of this box",
              text: "A box each",
            },
          ]}
        ...
        <Row label="Translation">
          <Toggles
            ...
            options={[
              {
                value: false,
                label: "Set exactly like the first language",
                text: "Same",
              },
              {
                value: true,
                label: "Give the second language a style of its own",
                text: "Its own",
              },
            ]}
            onPick={(own) =>
              patch({ secondary: own ? textStyleOf(element) : null })
            }
          />
        </Row>
        ...
        {element.secondary ? (
          <div className="space-y-3 rounded-studio border border-studio-border p-2">
            ...
            <TextStyleRows
              ...
              fonts={fonts}
              patch={(change) =>
                patch({ secondary: { ...element.secondary!, ...change } })
              }
            />
          </div>
        ) : null}
...
const PicturePicker = ({
  ...
  const shown = shelf
    ? pictures.filter((picture) => picture.folder === shelf)
    : pictures;
      ...
      options={[
        {
          ...
          label: "Fit the whole picture inside the box",
          text: "Fit",
        },
        {
          value: "cover" as const,
          label: "Crop it to fill the box",
          text: "Crop",
        },
        {
          value: "fill" as const,
          label: "Stretch it to the box, ignoring its shape",
          text: "Stretch",
        },
      ]}
      onPick={onPick}
      ...
      <Toggles
        ...
        options={[
          ...
          },
          {
            ...
            label: "A picture, clipped to the shape",
            Icon: ImageIcon,
          },
        ]}
...
const PictureInspector = ({
  ...
  <>
    ...
    <p className="text-[10px] leading-snug text-studio-faint">
      Pictures stay on this machine. A projector pulls one over the same
      connection it pulls a background over.
    </p>
  </>
);
...
export const TemplateEditor = ({
  ...
  const edit = useCallback(
    (key: string, next: (current: SlideTemplate) => SlideTemplate) => {
      const now = Date.now();
      const open = gesture.current?.key === key;
      const held =
        open &&
        (holding.current || now - (gesture.current?.at ?? 0) < GESTURE_MS);

      gesture.current = { key, at: now };

      setHistory((current) =>
        held
          ? amend(current, next(current.present))
          : commit(current, next(current.present)),
      );
    },
    [],
  );
  ...
  const bibleLangs: Lang[] = settings.langOrder.filter(
    (lang) => settings.enabled[lang],
  );

  const armed: string[] =
    stream || lyrics ? [] : bibleLangs.map((lang) => labelOf(lang));
  ...
  const style: ProjectorStyle = {
    ...
    enabled: lyrics
      ? {}
      : Object.fromEntries(canvasLangs.map((lang) => [lang, true])),
    ...(stream ? { lyricsLang: "sample-1" } : {}),
    ...(lyrics
      ? { lyricsTemplate: draft, lyricsLook: CUSTOM_LOOK }
      : { look: CUSTOM_LOOK }),
  };
  ...
  const replace = (id: string, change: Partial<TemplateElement>) =>
    edit(`${id}:${Object.keys(change).join(",")}`, (current) => ({
      elements: current.elements.map((one) =>
        one.id === id ? ({ ...one, ...change } as TemplateElement) : one,
      ),
    }));
  ...
  const duplicate = (id: string) =>
    paste(draft.elements.find((one) => one.id === id) ?? null);
  ...
  const onPointerMove = (event: React.PointerEvent) => {
    ...
    if (drag.handle === "turn") {
      ...
      const turned =
        drag.rotation +
        angleFrom(cx, cy, event.clientX, event.clientY) -
        drag.grabbed;

      replace(drag.id, {
        rotation: event.shiftKey
          ? snapAngle(turned)
          : Math.round(normalizeAngle(turned)),
      });
    ...
    if (drag.handle) {
      ...
      replace(drag.id, {
        frame: resizeBy(
          drag.frame,
          drag.handle,
          local.dx / drag.box.width,
          local.dy / drag.box.height,
        ),
      });
    ...
    const others = draft.elements
      .filter((one) => one.id !== drag.id)
      .map((one) => one.frame);
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      replace(element.id, {
        frame: clampToFrame(
          moveBy(element.frame, by[0] * distance, by[1] * distance),
        ),
      });
    };
  ...
  const save = () => {
    ...
    update({
      customTemplates: settings.customTemplates.map((row) =>
        row.id === id
          ? { ...row, name: name.trim() || "Custom", template: draft }
          : row,
      ),
    });
    onClose();
  ...
  const discard = () => {
    update({
      customTemplates: settings.customTemplates.filter((row) => row.id !== id),
    });
    onClose();
  };
      ...
      >
        <header className="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-studio-border px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <AddMenu
              disabled={draft.elements.length >= MAX_ELEMENTS}
              onAdd={add}
            />
            ...
            >
              ...
              {draft.elements.map((one) => (
                <div
                  ...
                  style={{
                    ...boxStyle(one.frame, one.rotation),
                    cursor: "move",
                  }}
                  className={cn(
                ...
                >
                  {HANDLES.map((handle) => (
                    <div
                      ...
                      role="presentation"
                      onPointerDown={(event) =>
                        startDrag(event, element.id, handle)
                      }
                  ...
                  <div
                    ...
                    onPointerDown={(event) =>
                      startDrag(event, element.id, "turn")
                    }
          ...
          >
            {!element ? (
              <>
                    ...
                    {sortable.items.map((one, index) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1 truncate text-[11px] text-studio-muted">
                            {nameOf(one)}
                          </span>
                        </button>
                ...
                <p className="text-[10px] leading-snug text-studio-faint">
                  Pick a box to change it. Arrow keys nudge the selection, ⌘C
                  and ⌘V copy it, Delete removes it.
                </p>
              </>
            ) : (
              <>
                    ...
                    {EDGES.map(({ edge, label, Icon }, index) => (
                      <Fragment key={edge}>
                        {index === 3 ? (
                          <span
                            aria-hidden
                            className="mx-0.5 h-4 w-px bg-studio-border"
                          />
                        ) : null}
          ...
          <div className="flex items-center gap-2">
            {locked ? (
              <p className="mr-1 max-w-64 text-[11px] leading-relaxed text-studio-muted">
                {limitMessage("custom_templates")} Draw all you like here — it
                just cannot be kept.
              </p>
            ) : null}
